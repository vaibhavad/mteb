...
def get_borda_count_df(df):
    borda_df = pd.DataFrame()
    borda_df["Model"] = df["Model"]
    for col in df.columns:
        if col in ["Model", "Overall"]:
            continue
        borda_df[col] = df[col].rank(method="min", ascending=False)
    
    borda_df["Overall"] = borda_df.iloc[:, 1:].sum(axis=1)

    # make Model column first, Overall column second
    cols = borda_df.columns.tolist()
    cols = ["Model", "Overall"] + [col for col in cols if col not in ["Model", "Overall"]]
    borda_df = borda_df[cols]
    # order by Overall
    borda_df = borda_df.sort_values("Overall", ascending=True)
...
def main():
    ...
    borda_df = get_borda_count_df(df)
    ...
    # print top 20 in markdown
    print(rank_df.head(20).to_markdown(index=False))
